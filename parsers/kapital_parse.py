...
FAILED_LOG = open("failed_extractions_kapital.txt", "a")
...
for root, dirs, files in os.walk(RAW_ROOT):
    ...
    for fname in files:
        ...
        # PDF: extract using best available method per report/year
            ...
            if year and year >= 2024 and ("other_general" in report_type or "distribution" in report_type):
                with pdfplumber.open(in_path) as pdf:
                    ...
                    print(f"[PDF] {in_path} (Plumber, {n_tables} tables) -> {out_xlsx}")
                    FAILED_LOG.write(f"[PDF] Plumber extracted {n_tables} tables for {in_path}\n")
            else:
