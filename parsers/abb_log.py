...
for root, dirs, files in os.walk(RAW_ROOT):
    ...
    for fname in files:
        ...
        if not os.path.exists(expected_excel):
            ...
            if (
                "risk_reports" in rel_path
                and (("currency_risk" in base.lower()) or ("credit_risk" in base.lower()))
                and ext.lower() == ".pdf"
            ):
                try:
                    xl = pd.ExcelFile(expected_excel)
...
print("\nDONE! FULL RAW-PROCESSED + WARNINGS LOG WRITTEN TO boss_report_abb.txt\n")
