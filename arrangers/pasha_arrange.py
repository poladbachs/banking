...
RAW_BANK = os.path.join("raw_data", "pasha_bank")
PROCESSED_ROOT = os.path.join("processed_data", "pasha_bank")
...
def arrange_files(root_folder, processed_root):
    for fname in os.listdir(root_folder):
        ...
        if not period or not out_name:
            ...
            continue
        # PDF files go to raw_data/pasha_bank/<period>/
        if ext == "pdf":
            target_dir = os.path.join(root_folder, period)
        # Excel files go to processed_data/pasha_bank/<period>/
        else:
